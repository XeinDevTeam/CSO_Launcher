...
#define PACKET_UMSG_PARSE_SIG_CSNZ "\x55\x8B\xEC\x6A\x00\x68\x00\x00\x00\x00\x64\xA1\x00\x00\x00\x00\x50\xB8\x00\x00\x00\x00\xE8\x00\x00\x00\x00\xA1\x00\x00\x00\x00\x33\xC5\x89\x45\x00\x56\x57\x50\x8D\x45\x00\x64\xA3\x00\x00\x00\x00\x8B\xF1\x89\xB5\x00\x00\x00\x00\x8B\x45"
#define PACKET_UMSG_PARSE_MASK_CSNZ "xxxx?x????xx????xx????x????x????xxxx?xxxxx?xx????xxxx????xx"
...
CreateHook(__cdecl, char, InitSocketManager, char* szFileName, char a2, char a3)
{
    return g_pfnInitSocketManager(szFileName, a2, a3);
}
...
void Hook(HMODULE hEngineModule, HMODULE hFileSystemModule)
{
    ...
    if (!g_bNoNGHook)
    {
        find = FindPattern("\x55\x8B\xEC\x83\xEC\x24\xA1\x00\x00\x00\x00\x33\xC5\x89\x45\x00\x53\x56\x57\xEB", "xxxxxxx????xxxx?xxxx", g_dwEngineBase, g_dwEngineBase + g_dwEngineSize, NULL);
        if (!find)
